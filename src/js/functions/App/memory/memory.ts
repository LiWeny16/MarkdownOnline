...
const DB_CONFIG = {
  ...
  version: 2,
  stores: {
    users_md: { keyPath: "uuid" } as StoreConfig,
    users_img: { 
      keyPath: "uuid", 
 ...
 */
async function initMdContentDB(): Promise<IDBDatabase> {
  try {
    // 尝试以正常版本打开数据库
 ...
 */
async function repairDatabaseByRecreating(): Promise<void> {
  // 尝试备份现有数据
 ...
 */
async function tryBackupExistingData(): Promise<{
  hasData: boolean
  textData?: any[]
  imageData?: any[]
}> {
  try {
    ...
    const backup: any = { hasData: false }
 ...
 * @description 恢复备份数据
 */
async function restoreBackupData(backup: any): Promise<void> {
 ...
 */
function createOrRepairStores(db: IDBDatabase, event: IDBVersionChangeEvent): void {
...
export async function fillInMemoryText(md: MD): Promise<void> {
...
export async function fillInMemoryImg(base64: string, timeStamp: number): Promise<void> {
  await safeDBOperation(
    async (db) => {
...
export async function fillInMemoryImgs(base64Arr: string[], timeStamp: number): Promise<void> {
  await safeDBOperation(
    async (db) => {
      const promises = base64Arr.map((base64, i) => 
        updateDB(db, "users_img", { uuid: timeStamp + i, imgBase64: base64 })
      )
...
export async function readMemoryImg(indexName: string, indexValue: any): Promise<any[]> {
  return safeDBOperation(
    async (db) => {
 ...
 */
export async function checkAndRepairDatabase(): Promise<{
  isHealthy: boolean
  issues: string[]
  repaired: boolean
}> {
  ...
  try {
    ...
    issues.push(`数据库检查失败: ${(error as Error).message}`)
    return { isHealthy: false, issues, repaired: false }
  } finally {
    if (db) {
      db.close()
