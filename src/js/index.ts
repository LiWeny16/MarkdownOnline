...
function updateMarkdown(newMarkdown: string) {
  ...
  patches.forEach(([operation, text]) => {
    if (operation === 1) {
      ...
      window.IncrementalDOM.patch(
        ...
        () => (markdownParser() as any).renderToIncrementalDOM(text, {})
      );
    } else if (operation === -1) {
      // 删除内容 -> 找到对应 DOM 节点并移除
...
// 移除 Markdown 变化的部分
function removeMarkdownContent(text: string) {
  const viewArea = document.getElementById("view-area");
  if (!viewArea) return;

  Array.from(viewArea.childNodes).forEach((node: any) => {
...
let originalElementOpen: Function | null = null;
...
export async function reactTableCustom() {
  ...
  window.IncrementalDOM.elementOpen = function (tagName: string, key: string, statics: any, ...attrs: any[]) {
    // 将属性数组转换为对象格式
    const attributes = attrs.reduce((acc: Record<string, any>, _, index, arr) => {
      if (index % 2 === 0) {
    ...
    if (classValue.includes('react-table')) {
      // 创建纯净的DOM元素
      const element = originalElementOpen!.call(
        this,
        tagName,
        key,
        statics,
...
const renderTables = async () => {
  ...
  elements.forEach((element) => {
    // 如果元素已经挂载过React根，跳过
    if (reactRoots.has(element)) return;
