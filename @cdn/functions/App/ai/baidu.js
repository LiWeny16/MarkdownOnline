 ...
 * 使用云函数调用百度翻译 API
 */
export async function translateWithBaidu(options) {
    try {
 ...
 */
export async function quickTranslate(text, targetLang, sourceLang = "auto") {
...
export async function batchTranslate(texts, targetLang, sourceLang = "auto") {
    try {
        ...
        const promises = texts.map(text => translateWithBaidu({
            text,
            targetLang,
            sourceLang,
        }));
 ...
 * 自动检测语言并翻译
 */
export async function autoTranslate(text, preferredTargetLang = "en") {
    try {
        const response = await fetch(CLOUD_FUNCTION_URL, {
            method: "POST",
            headers: {
