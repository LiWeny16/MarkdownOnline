...
function replaceSelection(e, leftStr, rightStr) {
    ...
    if (start == end) {
        ...
        let temp = e.value.substr(0, start) +
            leftStr +
...
function replaceMonacoSelection(newText = "nihao") {
    ...
    _editor.executeEdits("my-replace", [
        {
            range: new window.monaco.Range(currentSelection.startLineNumber, currentSelection.startColumn, currentSelection.endLineNumber, currentSelection.endColumn),
            text: newText,
...
function replaceMonacoInRange(startLineNumber, startColumn, endLineNumber, endColumn, newText) {
    ...
    _editor.executeEdits("my-replace", [
        {
            range: new window.monaco.Range(startLineNumber, startColumn, endLineNumber, endColumn),
            text: newText,
            forceMoveMarkers: true,
        },
    ]);
