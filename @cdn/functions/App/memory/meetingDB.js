 ...
 * 保存会议记录
 */
export async function saveMeetingRecord(meeting) {
    try {
 ...
 * 获取所有会议记录
 */
export async function getAllMeetingRecords() {
    try {
        const db = await openMeetingDB();
 ...
 * 根据ID获取会议记录
 */
export async function getMeetingRecordById(id) {
    try {
        const db = await openMeetingDB();
        const transaction = db.transaction([STORE_NAME], "readonly");
        const objectStore = transaction.objectStore(STORE_NAME);
 ...
 * 更新会议记录
 */
export async function updateMeetingRecord(meeting) {
    return saveMeetingRecord(meeting);
}
/**
 * 清空所有会议记录
 */
export async function clearAllMeetingRecords() {
    try {
        const db = await openMeetingDB();
        const transaction = db.transaction([STORE_NAME], "readwrite");
        const objectStore = transaction.objectStore(STORE_NAME);
 ...
 * 搜索会议记录
 */
export async function searchMeetingRecords(keyword) {
    try {
        const allMeetings = await getAllMeetingRecords();
        const lowerKeyword = keyword.toLowerCase();
        return allMeetings.filter((meeting) => {
            // 搜索标题、消息内容、总结
            return (meeting.title.toLowerCase().includes(lowerKeyword) ||
                meeting.summary?.toLowerCase().includes(lowerKeyword) ||
                meeting.messages.some((msg) => msg.text.toLowerCase().includes(lowerKeyword)));
        });
    }
    catch (error) {
