 ...
 * @description 快速标题1-4
 */
export default function exeHeadingAction(editor, monaco, level) {
    const selection = editor.getSelection();
    const range = new monaco.Range(selection.startLineNumber, 1, selection.startLineNumber, 1);
    ...
    if (match && match[1].length === level) {
        ...
        const replaceRange = match
            ? new monaco.Range(selection.startLineNumber, 1, selection.startLineNumber, match[0].length + 1)
            : range;
        editor.executeEdits("", [
            {
                range: replaceRange,
