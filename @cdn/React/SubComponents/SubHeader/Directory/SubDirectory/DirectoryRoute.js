...
function renderTreeItems(items, setActiveId, t) {
    return items.map((item) => (_jsx(CustomTreeItem, { itemId: item.id, label: t(item.label), slots: { endIcon: item.icon }, onClick: () => { if (item.id.slice(-1) != "x") {
            setActiveId([item.id]);
...
export default function DirectoryRoute(props) {
    ...
    return (_jsx(Box, { className: "transparent-scrollbar", sx: {
            ...
            borderRight: "solid rgba(0, 0, 0, 0.12)",
                ...
                minHeight: 270,
                maxWidth: 300,
            }, children: renderTreeItems(treeData, props.setActiveId, t) }) }));
}
